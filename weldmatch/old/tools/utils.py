...
from tools.exceptions import EmptyTensorError
...
def preprocess_image(image, preprocessing=None):
    ...
    if preprocessing is None:
        pass
    elif preprocessing == 'caffe':#预留caffe的接口暂时不用
        # RGB -> BGR
        image = image[:: -1, :, :]
        ...
        image /= 255.0
        mean = np.array([0.251573, 0.251573, 0.251573])
        std = np.array([0.224095, 0.224095, 0.224095])
