...
from anchors import anchor_targets_bbox, bbox_transform
from image import (
    TransformParameters,
...
class Generator(object):
    """ Abstract generator class.#应该在这里实现了数据处理方式
    """
    ...
    def __init__(
        ...
        """
        self.transform_generator    = transform_generator #图像变换，拉伸、旋转等操作。在csv_generate.CSVGenerator()中传入
        self.batch_size             = int(batch_size)
        self.group_method           = group_method #一个batch内数据的组成方式
    ...
    def size(self):
        """ Size of the dataset. 在子类方法中实现class CSVGenerator(Generator)
        """
    ...
    def image_aspect_ratio(self, image_index):
        """ Compute the aspect ratio for an image with image_index.实现在class CSVGenerator(Generator)
        """
    ...
    def preprocess_image(self, image):
        ...
        """
        return preprocess_image(image)# 减去ImageNet mean
    ...
    def group_images(self):
        ...
        order = list(range(self.size()))#0-文件数目list
        if self.group_method == 'random':
            random.shuffle(order)
        elif self.group_method == 'ratio':
            order.sort(key=lambda x: self.image_aspect_ratio(x))#按图片长宽比排序class CSVGenerator(Generator):float(image.width) / float(image.height)

        # divide into groups, one group = one batch          0,4                             0,4,8,12...  len(order)
        self.groups = [[order[x % len(order)] for x in range(i, i + self.batch_size)] for i in range(0, len(order), self.batch_size)]
        #self.groups = [[a1,b1,c1,d1],[a2,b2,c2,d2]......]
    ...
    def compute_input_output(self, group):
        ...
        image_group       = self.load_image_group(group)#[BRG numpy矩阵]
        annotations_group = self.load_annotations_group(group) # boxes = np.zeros((len(annots), 5));[boxes...]

        # check validity of annotations检查注释
        image_group, annotations_group = self.filter_annotations(image_group, annotations_group, group)

        # perform preprocessing steps预处理
        ...
        return inputs, targets# image_batch  [regression_batch, labels_batch]
    ...
    def next(self):
        ...
        with self.lock:
            ...
            self.group_index = (self.group_index + 1) % len(self.groups) #当(self.group_index + 1)==len(self.groups)时self.group_index重新置0
