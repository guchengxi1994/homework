...
def anchor_targets_bbox(
    ...
    **kwargs
    ...
    """
    anchors = anchors_for_shape(image_shape, **kwargs)#结构为np.append(all_anchors, shifted_anchors, axis=0)
    ...
    if annotations.shape[0]:
        ...
        # no annotations? then everything is background #非有效box都作为背景学习
        labels[:] = 0
        annotations = np.zeros((anchors.shape[0], annotations.shape[1]))

    # ignore annotations outside of image 忽略超出图像的box
